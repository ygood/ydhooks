...
describe('test useInterval', () => {
  it('基本測試', async () => {
    const { result } = renderHook(() => useLocalStorageState('age', { defaultValue: 18 }));
  ...
  it('复杂数据測試', async () => {
    const { result } = renderHook(() =>
      useLocalStorageState('JSON1', { defaultValue: { age: 18, name: 'yanlei' } }),
    );
    ...
    expect(localStorage.getItem('JSON1')).toBeDefined();
  });

  it('复杂自定义序列化函数', async () => {
    const { result } = renderHook(() =>
      useLocalStorageState('JSON2', { defaultValue: 18, serializer, deserializer }),
    );
    expect(result.current[0]).toEqual(18);
    act(() => {
      result.current[1](19);
    });
    expect(localStorage.getItem('JSON2')).toEqual('20');
  });
});
